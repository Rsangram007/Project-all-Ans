...
const interns = async function (req, res) {
  try {
    ...
    if (!validator.isValidRequestBody(data)) {
      return res.status(400).send({ status: false, msg: "incomplete request data" });}
    //--mandatory field--//
    ...
    if (!validator.isValidName(name)) {
      return res.status(400).send({   status: false, msg: " Name is required and first character must be capital...!", });
    }
    ...
    let internData = await internModel.create(data);
     ["_id", "updatedAt", "createdAt", "__v"].forEach(   (x) => delete internData._doc[x]  );
    res.status(201).send({  status: true,   data: internData,  message: "Intern created successfully..",});
  } catch (err) {
